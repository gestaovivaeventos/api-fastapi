        ...
        with conn.cursor() as cursor:
            ...
            query = """
                SELECT
                    ...
                    f.id AS id_fundo, '' AS nm_fundo, c.nm_curso AS curso_fundo, '' AS tp_servico,
                    ...
                    it.nm_instituicao, '' AS fl_ativo, '' AS tipo_cliente
                FROM tb_fundo f
                ...
                WHERE u.categoria = '2' AND f.tipocliente_id IN (15, 17) AND i.dt_cadastro >= '2019-01-01' AND f.is_fundo_teste IS FALSE
                AND i.nu_status NOT IN (11, 9, 8, 13, 14) AND f.is_assessoria_pura IS FALSE
                AND (i.dt_cadastro <= '2024-03-08' OR (i.dt_cadastro > '2024-03-08'
                AND i.id NOT IN (SELECT i2.id FROM tb_integrante i2 WHERE i2.forma_adesao = 6 AND i2.dt_cadastro > '2024-03-08')))
                ORDER BY i.dt_cadastro
                LIMIT %s OFFSET %s
            """
